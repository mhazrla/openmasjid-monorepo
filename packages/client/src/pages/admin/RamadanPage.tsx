...
import { 
    Loader2, Moon, Save, Edit3, User, Utensils, Calendar
} from 'lucide-react';
...
const ScheduleRow = memo(({ schedule, index, totalRows }: ScheduleRowProps) => 
{
    ...
    const [formData, setFormData] = useState({
        ...
        // Lelang Buka Puasa
        iftarTarget: schedule.iftarTarget || 0,
        iftarCurrent: schedule.iftarCurrent || 0,

        // Lelang Sahur I'tikaf
        itikafTarget: schedule.itikafTarget || 0,
        itikafCurrent: schedule.itikafCurrent || 0,
    });
    ...
    const handleSave = useCallback(() => 
    {
        updateSchedule({
            ...
            iftarTarget: Number(formData.iftarTarget) || 0,
            iftarCurrent: Number(formData.iftarCurrent) || 0,
            ...
            <td className="px-4 py-3 min-w-[280px]">
                <div className="flex flex-col gap-3">
                    <UserSelect 
                        ...
                        placeholder="Pilih Pemateri Buka"
                    />
                    <MinimalInput 
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-1 border-t border-slate-100 pt-2">
                        <div className="text-center space-y-1">
                            <span className="text-[9px] text-slate-400 uppercase tracking-widest font-semibold flex items-center justify-center gap-1"><Utensils className="w-3 h-3"/> Target Buka</span>
                            <div className="relative">
                                <span className="absolute right-2 top-1.5 text-[10px] text-slate-400 pointer-events-none">box</span>
                                <MinimalInput type="number" min={0} className="text-center font-mono pr-6 font-bold text-slate-700" value={formData.iftarTarget} onChange={(e) => handleChange('iftarTarget', e.target.value)} />
                            </div>
                        </div>
                        <div className="text-center space-y-1">
                            <span className="text-[9px] text-emerald-500 uppercase tracking-widest font-semibold flex items-center justify-center gap-1">Terkumpul</span>
                            <div className="relative">
                                <span className="absolute right-2 top-1.5 text-[10px] text-emerald-400 pointer-events-none">box</span>
                                <MinimalInput type="number" min={0} className="text-center font-mono pr-6 font-bold text-emerald-600 bg-emerald-50/50" value={formData.iftarCurrent} onChange={(e) => handleChange('iftarCurrent', Number(e.target.value))} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <td className="px-4 py-3 min-w-[240px]">
                <div className="flex flex-col gap-3 justify-center h-full">
                    <div className="grid grid-cols-2 gap-2 mt-4">
                        <div className="text-center space-y-1">
                            <span className="text-[9px] text-slate-400 uppercase tracking-widest font-semibold flex items-center justify-center gap-1"><Moon className="w-3 h-3"/> Target Sahur</span>
                            <div className="relative">
                                <span className="absolute right-2 top-1.5 text-[10px] text-slate-400 pointer-events-none">box</span>
                                <MinimalInput type="number" min={0} className="text-center font-mono pr-6 font-bold text-slate-700" value={formData.itikafTarget} onChange={(e) => handleChange('itikafTarget', Number(e.target.value))} />
                            </div>
                        </div>
                        <div className="text-center space-y-1">
                            <span className="text-[9px] text-emerald-500 uppercase tracking-widest font-semibold flex items-center justify-center gap-1">Terkumpul</span>
                            <div className="relative">
                                <span className="absolute right-2 top-1.5 text-[10px] text-emerald-400 pointer-events-none">box</span>
                                <MinimalInput type="number" min={0} className="text-center font-mono pr-6 font-bold text-emerald-600 bg-emerald-50/50" value={formData.itikafCurrent} onChange={(e) => handleChange('itikafCurrent', Number(e.target.value))} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
                <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-20">
                    ...
                    <div className="flex gap-2">
                        {/* Status legends removed since everything is target vs current driven */}
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50/80 backdrop-blur sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-4 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Buka Puasa (Kajian & Lelang)</th>
                                <th className="px-4 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Sahur I'tikaf (Lelang)</th>
